...
@export var fire_rate : float = 0.4   # 连射间隔（每秒2.5发）
var fire_timer : float = 0.0          # 连射计时器
...
func _physics_process(_delta: float) -> void:	#这玩意是固定成60fps，好像挺牛逼的
	...
	if not is_game_over:
		...
		if velocity == Vector2.ZERO:
			...
			idle_timer = 0.0
			fire_timer = 0.0          # 移动时重置连射计时
		# 🔫 自动射击（只在非翻滚时）
		if not is_rolling:
			fire_timer += _delta
			if fire_timer >= fire_rate and idle_timer >= fire_delay:
				$Node2D/firesound.play()
				var bullet_node = bullet_scene.instantiate()
				bullet_node.position = position + Vector2(17, 7)
				get_tree().current_scene.add_child(bullet_node)
				fire_timer = 0.0      # 重置下一发间隔
				# 如果想打一发后重新等 0.2s，就取消下面这行注释：
				# idle_timer = 0.0
		move_and_slide()
...
func _on_fire() -> void:
	print("fire被调用, idle_timer=", idle_timer, " 条件=", idle_timer >= fire_delay)
	if is_game_over or velocity != Vector2.ZERO or idle_timer < fire_delay: # 如果速度不等于0
		return #中断代码
